...
import { decodeFulfilment, EvmLog, receiptCommit, REQUEST_FULFILLED_TOPIC } from "./receipt";
...
const getLogs = async (url: string, address: string, fromBlock: number, toBlock: number): Promise<EvmLog[]> =>
  (await rpc(url, "eth_getLogs", [{ address, fromBlock: `0x${fromBlock.toString(16)}`, toBlock: `0x${toBlock.toString(16)}`, topics: [REQUEST_FULFILLED_TOPIC] }])) as EvmLog[];
...
async function scanChain(chainId: number, cfg: WatchConfig): Promise<{ logs: number; enqueued: number } | null> {
  ...
  const receipts = logs
    .map((l) => decodeFulfilment(l, { chain: router.name, chainId }))
    .filter((f): f is NonNullable<typeof f> => f !== null)
    .map(receiptCommit);
  const enqueued = await enqueueFulfilments(receipts, nowSeconds());
  await setCursor(chainId, to, nowSeconds());
  if (logs.length > 0) {
    log(`scan[${router.name}] blocks ${from}-${to}: ${logs.length} fulfilments, ${enqueued} new enqueued (cursor→${to}/${head})`);
  }
